...
public class AddSiteAdminsServlet extends RequiresLoginServlet {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -3576945237442884589L;
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		try {
			...
			if("Add".equals(req.getParameter("action"))) {
				...
				getApp().getUserManager().addSiteAdmins(playerIDs);
			} else if("Remove".equals(req.getParameter("action"))) {
				int[] playerIDs = convertToIntegers(req.getParameterValues("player"));
				getApp().getUserManager().removeSiteAdmins(user, playerIDs);
			}
