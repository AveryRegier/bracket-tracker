 ...
 */
public class GameFeederLoadBroker extends LoadBroker {

	/**
	 * @param sp
	 */
	public GameFeederLoadBroker(SingletonProvider sp) {
		super(sp);
	}
	...
	protected void processResults(ResultSet set) throws SQLException {
		while(set.next()) {
			...
			int feederGameNodeOID = set.getInt("FEEDER_GAME_NODE_ID");
			int oponentOID = set.getInt("OPPONENT_ID");
			int tournamentTypeOID = set.getInt("TOURNAMENT_TYPE_ID");
			sp.getSingleton().getTournamentTypeManager().loadGameFeeder(
				gameNodeOID, feederGameNodeOID, oponentOID, tournamentTypeOID
			);
		}
	}
