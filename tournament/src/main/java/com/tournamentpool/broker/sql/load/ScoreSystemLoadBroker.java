 ...
 */
public class ScoreSystemLoadBroker extends LoadBroker {

	/**
	 * @param sp
	 */
	public ScoreSystemLoadBroker(SingletonProvider sp) {
		super(sp);
	}
	...
	protected void processResults(ResultSet set) throws SQLException {
		while(set.next()) {
			...
			sp.getSingleton().getScoreSystemManager().loadScoreSystem(scoreSystemOID, name);
		}
	}
