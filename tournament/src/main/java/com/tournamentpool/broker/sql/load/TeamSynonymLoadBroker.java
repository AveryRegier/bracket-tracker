 ...
 * @author Avery Regier 
 */
public class TeamSynonymLoadBroker extends LoadBroker {

	/**
	 * @param sp
	 */
	public TeamSynonymLoadBroker(SingletonProvider sp) {
		super(sp);
	}
	...
	protected void processResults(ResultSet set) throws SQLException {
		while(set.next()) {
			...
			sp.getSingleton().getTeamManager().loadTeamSynonym(teamOID, name);
		}
	}
}
