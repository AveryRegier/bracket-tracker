 ...
 */
public class TournamentAdminLoadBroker extends LoadBroker {

	/**
	 * @param sp
	 */
	public TournamentAdminLoadBroker(SingletonProvider sp) {
		super(sp);
	}
	...
	protected void processResults(ResultSet set) throws SQLException {
		while(set.next()) {
			...
			int adminOID = set.getInt("ADMIN_ID");
			MainTournament mainTournament = 
				((MainTournament) sp.getSingleton().getTournamentManager()
					.getTournament(tournamentOID));
			if(mainTournament != null) {
				mainTournament.loadAdmin(adminOID);
			}
		}
	}
