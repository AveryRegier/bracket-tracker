 ...
 */
public class GameNodeLoadBroker extends LoadBroker {

	/**
	 * @param sp
	 */
	public GameNodeLoadBroker(SingletonProvider sp) {
		super(sp);
	}
	...
	protected void processResults(ResultSet set) throws SQLException {
		while(set.next()) {
			...
			sp.getSingleton().getTournamentTypeManager().loadGameNode(gameNodeOID, levelOID, gameFeederTypeOID);
		}
	}
}
