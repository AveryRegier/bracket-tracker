 ...
 * @author Avery Regier 
 */
public class LeagueLoadBroker extends LoadBroker {

	/**
	 * @param sp
	 */
	public LeagueLoadBroker(SingletonProvider sp) {
		super(sp);
	}
	...
	protected void processResults(ResultSet set) throws SQLException {
		while(set.next()) {
			...
			sp.getSingleton().getTeamManager().loadLeague(leagueOID, name);
		}
	}
}
