 ...
 */
public class GameScoreLoadBroker extends LoadBroker {

	/**
	 * @param sp
	 */
	public GameScoreLoadBroker(SingletonProvider sp) {
		super(sp);
	}
	...
	protected void processResults(ResultSet set) throws SQLException {
		...
		//	int gameOID = set.getInt("GAME_ID");
			...
			int opponentOID = set.getInt("OPPONENT_ID");
			int score = set.getInt("SCORE");
			
			sp.getSingleton().getTournamentManager().loadGameScore(
			/*	gameOID,*/ gameNodeOID, tournamentOID, opponentOID, score
			);
		}
	}
