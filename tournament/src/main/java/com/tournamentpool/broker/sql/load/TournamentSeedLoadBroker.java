 ...
 */
public class TournamentSeedLoadBroker extends LoadBroker {

	/**
	 * @param sp
	 */
	public TournamentSeedLoadBroker(SingletonProvider sp) {
		super(sp);
	}
	...
	protected void processResults(ResultSet set) throws SQLException {
		while(set.next()) {
			...
			sp.getSingleton().getTournamentManager().loadTournamentSeed(
				tournamentOID, teamOID, seedOID
			);
		}
	}
