 ...
 */
public class OpponentLoadBroker extends LoadBroker {

	/**
	 * @param sp
	 */
	public OpponentLoadBroker(SingletonProvider sp) {
		super(sp);
	}
	...
	protected void processResults(ResultSet set) throws SQLException {
		while(set.next()) {
			...
			int sequence = set.getInt("SEQUENCE");
			sp.getSingleton().getTournamentTypeManager().loadOponent(
				oponentOID,
				tournamentTypeOID,
				name,
				sequence
